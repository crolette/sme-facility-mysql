...
class AssetsSheet implements FromQuery, WithMapping, Responsable, WithHeadings, ShouldAutoSize, WithStyles, WithColumnFormatting
{
    use Exportable;
    ...
    public function map($asset): array
    {
        ...
        return [
            ...
            'site',
            '',
            $asset->is_mobile ? $asset->location->full_name: $asset->location->name,
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $validation->setErrorStyle(\PhpOffice\PhpSpreadsheet\Cell\DataValidation::STYLE_INFORMATION);
        $validation->setShowDropDown(true);
        $validation->setFormula1('"site,building,floor,room"');
        $sheet->setDataValidation('H3:H9999', clone $validation);

        $validation = $sheet->getDataValidation('I3');
        $validation->setType(\PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_LIST);
        $validation->setErrorStyle(\PhpOffice\PhpSpreadsheet\Cell\DataValidation::STYLE_INFORMATION);
        $validation->setShowDropDown(true);
        $validation->setFormula1('=INDIRECT(H3&"s")');
