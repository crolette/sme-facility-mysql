...
class ApiImportContractsController extends Controller
{
    ...
    public function store(ImportFileRequest $request)
    {
        ...
        if (!str_contains($validated['file']->getClientOriginalName(), 'contracts')) {
            return ApiResponse::error('Wrong file. The file name should include contracts');
        }
        ...
        try {
            ...
            Log::info('DISPATCH IMPORT CONTRACTS EXCEL JOB : ' . $directory . $fileName);
            ImportExcelContractsJob::dispatch(Auth::user(), $directory . $fileName);

            return ApiResponse::success('', 'Contracts will be imported, you will receive an email when it\'s done.');
