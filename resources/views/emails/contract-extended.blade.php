...
<head>
    <meta charset="utf-8">
    <title>{{ __('contracts.contract_extended') . ' : ' .  $contract->name . '(' . $contract->type->value . ') - ' . $contract->provider->name}}</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1 class="title">{{ __('contracts.contract_extended') . ' : ' . $contract->name . ' (' . $contract->type->value . ') - ' . $contract->provider->name}}</h1>
        </div>

        <div class="content">
            <div class="alert">
                          <strong>⚠️ {{__('contracts.contract_extended')}}</strong>
                    </div>
                    <div class="details">
