...
export default function FeaturesQrCode() {
            ...
            items.push(
                <details className="" open={i == 0 ? true : false} key={i}>
                    <summary className="cursor-pointer text-2xl font-bold">
                        <h3>{t(`website_features.qrcode.faq.${i + 1}.question`)}</h3>
                        <hr className="mt-3" />
                    </summary>
                    <p className="mt-6 text-lg">{t(`website_features.qrcode.faq.${i + 1}.answer`)}</p>
                </details>,
            );
        }
        return <>{items}</>;
    ...
    return (
        <WebsiteLayout>
            <Head title={t('website_features.qrcode.meta_title')}>
                <meta name="robots" content="index, follow"></meta>
                <meta property="title" content={t('website_features.qrcode.meta_title') + ' | ' + import.meta.env.VITE_APP_NAME} />
                <meta name="description" itemProp="description" property="description" content={t('website_features.qrcode.meta_description')} />
                ...
                <div className="container">
                    <div className="mx-auto grid h-full gap-10 px-4 py-32 md:grid-cols-[2fr_1fr] md:gap-10 md:px-10 lg:max-w-11/12">
                        <div className="flex flex-col items-center justify-center gap-10">
                            <h1 className="">
                                {t('website_features.qrcode.title')} <span className="font-extrabold">{t('website_features.qrcode.title-span')}</span>
                            </h1>
                            <h2 className="!text-xl">{t('website_features.qrcode.subtitle')}</h2>
                            <p className="">{t('website_features.qrcode.description')}</p>
                        ...
                        <div className="mx-auto my-auto">
                            <img src="/images/website/features/hero_qrcodes.png" alt="" className="blob h-auto max-w-72 shadow-2xl md:w-full" />
                        </div>
                    </div>
                </div>
            </section>
                        ...
                        <div className="grid gap-6 md:grid-cols-3">
                            <div className="from-website-border rounded-md bg-linear-to-t to-transparent p-0.5">
                                <div className="grid h-full grid-rows-[1fr_2fr] gap-4 rounded-md bg-white p-6">
                                    <h6 className="font-semibold">{t('website_features.qrcode.card-1.title')}</h6>
                                    <p>{t('website_features.qrcode.card-1.description')}</p>
                                </div>
                            </div>
                            <div className="from-website-border rounded-md bg-linear-to-t to-transparent p-0.5">
                                <div className="grid h-full grid-rows-[1fr_2fr] gap-4 rounded-md bg-white p-6">
                                    <h6 className="font-semibold">{t('website_features.qrcode.card-2.title')}</h6>
                                    <p>{t('website_features.qrcode.card-2.description')}</p>
                                </div>
                            </div>
                            <div className="from-website-border rounded-md bg-linear-to-t to-transparent p-0.5">
                                <div className="grid h-full grid-rows-[1fr_2fr] gap-4 rounded-md bg-white p-6">
                                    <h6 className="font-semibold">{t('website_features.qrcode.card-3.title')}</h6>
                                    <p>{t('website_features.qrcode.card-3.description')}</p>
                                </div>
                            </div>
                        </div>
                        <img src="/images/website/features/qrcodes.png" alt="" className="mx-auto w-3/4" />
