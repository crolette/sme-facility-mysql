...
export const AddressForm = ({ idPrefix, address, onChange }: AddressFormProps) => {
    ...
    return (
        <>
            ...
            <Input id={`${idPrefix}_street`} value={address?.street ?? ''}  required onChange={updateField('street')} placeholder="Rue de la facilité" />

            <Label htmlFor={`${idPrefix}_house_number`}>House Number</Label>
            <Input id={`${idPrefix}_house_number`} value={address?.house_number ?? ''}  required onChange={updateField('house_number')} placeholder="14" />
